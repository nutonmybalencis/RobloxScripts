...
local function loadLegitDist()
	...
	cfg.distancehitchancepoints = {
		{distance = 10, chance = 85}, {distance = 20, chance = 78}, {distance = 35, chance = 68},
		{distance = 50, chance = 58}, {distance = 75, chance = 48}, {distance = 100, chance = 38},
		{distance = 200, chance = 22}, {distance = 250, chance = 15}, {distance = 300, chance = 10},
...
local function isC4(pt)
	...
	return n == "explosive" or n == "c4" or n == "clientc4" or pn:find("c4") or n:find("c4")
end

local function onDescAdd(desc)
	if isC4(desc) then trackedC4[desc] = true end
	trackGrab(desc)
end
...
local function applyMicroCorr(tp, dt, tgt)
	...
	if microState.last ~= tgt then
		...
		local dir
		if hv.Magnitude > 0.5 then dir = hv.Unit
		else dir = Vector3.new(rng:NextNumber(-1, 1), rng:NextNumber(-0.3, 0.3), rng:NextNumber(-1, 1)).Unit end
		microState.off = dir * cfg.microCorrectionAmount
	end
	...
	if el >= dur then microState.active = false return tp end
	local t = el / dur
	local ease = (1 - t) * (1 - t)
	return tp + microState.off * ease
end
...
local function isOnTarget(tgt)
	...
	if not tgt or not tgt.Character then return true end
...
local function autoShoot(dt)
	...
	local gunFr = g:GetAttribute("FireRate") or 0
local fr = math.max(gunFr, cfg.autoshootdelay)
	...
	if tgt ~= lastAutoTarget then
		...
		if cfg.autoshootlinger and linger.active and now < linger.endT then
			if linger.lastTgt then
				...
				--// Linger shoots at LAST VISIBLE position, doesn't track
